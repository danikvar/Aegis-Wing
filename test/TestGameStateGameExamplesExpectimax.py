...
class TestGameStateGameExamples(unittest.TestCase):
    ...
    def test_game_example_1(self):
        ...
        # row 1, col = 7
        enemy_1 = EnemyMoveFireHeuristicAgent(4, 4)
    ...
    def test_game_example_3(self):
        ...
        state = self.gamestateInit
        state.max_enemies_at_any_given_time = 10
        # set small turns
        state.turns_left = 10
        # make a player and add it
        player = ExpectimaxAgent(1, 1, 0, 0, 2)
        ...
        enemy_1 = SimpleGoLeftAgent(0, 2)
        enemy_2 = SimpleGoLeftAgent(1, 5)
        enemy_3 = SimpleGoLeftAgent(2, 3)
        enemy_4 = SimpleGoLeftAgent(3, 4)
        enemy_5 = SimpleGoLeftAgent(4, 5)
        enemy_6 = SimpleGoLeftAgent(5, 6)
        enemy_7 = SimpleGoLeftAgent(6, 7)
        enemy_8 = SimpleGoLeftAgent(7, 8)
        enemy_9 = SimpleGoLeftAgent(8, 9)
        ...
        while True:
            ...
            for each_index in range(len(state.current_agents)):
                ...
                # print("STATE CURRENT AGENTS", state.current_agents)
                if (len(state.current_agents) <= 0) or (state.isWin()) or (state.isLose()) or (state.isGameOver()):
                    break
                elif (state.current_agents[len(state.current_agents) - 1].hasMoved() == True):
                    state.decrement_turn()
